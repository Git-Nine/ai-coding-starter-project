...
- ⚠️ **GRANT fix required (`supabase/migrations/20260618150000_proj3_grant_scans_privileges.sql`) — NOT yet applied.** Like PROJ-2's `users` table (BUG-7), `public.scans` was created without table-level privileges for `authenticated`/`service_role` (Supabase default privileges didn't apply), so **every authenticated scan read/write fails at runtime with `42501 permission denied`** regardless of RLS. Surfaced by PROJ-2's two-account harness. **Operator action:** run `grant select, insert, update, delete on public.scans to authenticated; grant all on public.scans to service_role;` in the SQL editor. Until then the scans list shows the empty state and saving errors (now a permission error, no longer a missing-table error). Convention going forward: every user-data table needs an explicit `GRANT ... TO authenticated`.
